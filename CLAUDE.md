...
printed to stderr. Keep the two in sync when changing the scan logic, with one
intentional exception: the `skew` column exists only in the Python script. See
`mobile/README.md`.
...
`skew` is 25-delta volatility skew, computed locally with scipy (`brentq` plus `scipy.stats.norm`) because no REST endpoint returns per-strike implied volatility or delta. It ignores dividends and approximates the forward with spot; see README.md for why, and do not "fix" that without measuring the effect on real quotes first. It is Python-only and deliberately absent from `mobile/lib/scan.ts`.
